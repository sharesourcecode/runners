...
#quest take help
		...
		close=$?
		[[ $close != 0 && $take6 = 0 ]] && {
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/take/6" -useragent="$(shuf -n1 $TMP/$c/.ua)" >$TMP/SRC ) &
			_timexit 10
			echo "$c" >$TMP/CAREER
		}
		[[ $close != 0 && $help6 = 0 ]] && {
			[[ $close = 0 ]] && _close
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/help/6" -useragent="$(shuf -n1 $TMP/$c/.ua)" >$TMP/SRC ) &
			_timexit 10
		}
...
#quest end deleteHelp
		...
		[[ $deleteHelp6 = 0 || $end6 = 0 ]] && {
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/deleteHelp/6" -useragent="$(shuf -n1 $TMP/$c/.ua)" >$TMP/SRC ) &
			_timexit 10
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/end/6" -useragent="$(shuf -n1 $TMP/$c/.ua)" >$TMP/SRC ) &
			_timexit 10
		}
	}
}
