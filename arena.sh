...
#quest help
	...
	grep -q '/quest/take/3' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/take/3" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 10
		echo "$c" >$TMP/ARENA3
	}	
	grep -q '/quest/take/4' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/take/4" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 10
		echo "$c" >$TMP/ARENA4
	}
	grep -q -o -P '(\W\W/span\W \Wspan class\W\Wgreen(.*)/clan/\d{1,4}/quest/\w{3,10}/3)' $TMP/SRC || {
		grep -q '/quest/help/3' $TMP/SRC && {
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/help/3" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
			_timexit 10
		}
	}
	grep -q -o -P '(\W\W/span\W \Wspan class\W\Wgreen(.*)/clan/\d{1,4}/quest/\w{3,10}/4)' $TMP/SRC || {
		grep -q '/quest/help/4' $TMP/SRC && {
			( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/help/4" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
			_timexit 10
		}
	}	
#arena
...
#quest delete help
	...
	grep -q '/quest/deleteHelp/3' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/deleteHelp/3" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 20
	}
	grep -q '/quest/deleteHelp/4' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/deleteHelp/4" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 20
	}
	grep -q '/quest/end/3' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/end/3" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 10
	}
	grep -q '/quest/end/4' $TMP/SRC && {
		( torsocks lynx -dump -cfg=$TMP/$c/cfg "http://furiadetitas.net/clan/1271/quest/end/4" -useragent="$(shuf -n1 $TMP/$c/.ua)" ) &
		_timexit 10
	}
#fault
